...
> **Wind AIMarket 金融能力发现器（meta-skill）** · 读 llm-wiki，帮 AI 命中对路的金融 skill
...
- 用户问金融问题 → AI 不确定用哪个 skill → 触发本 skill 查 wiki → 推荐对路的金融 skill + 安装命令
...
# 3. 读对应文档（远程优先 + 24h 缓存 + 本地兜底）
node scripts/cli.mjs read-doc categories/股票/行情.md

# 4. 强制刷新本地缓存（平台刚发新 skill 时用）
...
AI 在 SKILL.md 加载后会按 3 步走：`find-capability` → `read-doc` → 给用户答复（含安装命令）。
